...
if ($slink == "my") {
	echo "<h2 class='h2-head'><a href='/clanky/' title='Články'>Články</a></h2><h3><a href='/clanky/my/' title='Moje články'>Moje články</a></h3>";
...
<?php
	...
	//kontrola zalozky
	$sB = chBook();
	echo "<h2 class='h2-head'><a href='/clanky/' title='Články'>Články</a></h2>";
...
<?php
	...
	$lmt = 2;
	$sql = "SELECT nazev,nazev_rew,schvalenotime FROM 3_clanky WHERE sekce = '".$oA->sekce."' AND schvaleno = '1' AND schvalenotime > '$date' ORDER BY schvalenotime ASC LIMIT 2";
	...
	$sql = "SELECT nazev,nazev_rew,schvalenotime FROM 3_clanky WHERE sekce = '".$oA->sekce."' AND schvaleno = '1' AND schvalenotime < '$date' ORDER BY schvalenotime DESC LIMIT $lmt";
...
echo "<h2 class='h2-head'><a href='/clanky/' title='Články'>Články</a></h2><h3><a href='/clanky/' title='Články - chyba'>Error</a></h3>";
echo "	<p class='submenu'><a href='/clanky/' class='permalink' title='Zpět na výpis Článků'>Zpět do Článků</a></p>\n";
info("Hledaný článek nebyl v databázi nalezen.");

}elseif ($slink == "new" && !$LogedIn) {
echo "<h2 class='h2-head'><a href='/clanky/' title='Články'>Články</a></h2><h3><a href='/clanky/new/' title='Odeslat nový článek ke schválení'>Odeslat nový článek</a></h3>";
echo "	<p class='submenu'><a href='/clanky/' class='permalink' title='Zpět na výpis Článků'>Zpět do Článků</a></p>\n";
info("Tato sekce je vyhrazena jen registrovaným uživatelům.");
...
<label><span>Sekce</span><select name='sekce' style='width: 402px'><option value='0'>Povídky</option><option value='1'>Poezie</option><option value='2'>Úvahy</option><option value='3'>Recenze</option><option value='4'>Postavy</option><option value='5'>Ostatní</option><option value='7'>Rozhovory</option></select></label>
<label><span>Anotace</span><input type='text' name='anotace' value='' maxlength='250' /></label>
<div><label style="margin-bottom:0px;"><span>Text (povoleno html)</span></label><textarea rows='20' name='mess' /></textarea></div>
...
<?php
	...
	if (!$LogedIn) $sql = "SELECT c.id, c.nazev, c.nazev_rew, c.schvalenotime, u.login_rew, u.login, c.anotace, c.sekce, 0 AS unread_comms, c.hodnoceni, c.hodnotilo, 
...
DESC $limiter";
	else $sql = "SELECT c.id, c.nazev, c.nazev_rew, c.sekce, c.anotace, c.schvalenotime, u.login, u.login_rew, c.hodnotilo, c.hodnoceni,
(SELECT COUNT(*) FROM 3_comm_1 AS cm WHERE cm.aid = c.id) AS all_comms, (SELECT COUNT(*) FROM 3_visited_1 AS v, 3_comm_1 AS cm WHERE v.aid = c.id AND v.uid = ".$_SESSION['uid']." AND cm.aid = c.id AND cm.id > v.lastid) AS unread_comms,
(SELECT COUNT(*) FROM 3_visited_1 AS vv WHERE vv.aid = c.id AND vv.uid = ".$_SESSION['uid'].") AS v_uid
FROM 3_clanky AS c, 3_users AS u
WHERE c.schvaleno = 1 AND u.id = c.autor $sqlAdd $autorSQL 
ORDER BY c.schvalenotime 
DESC $limiter";
