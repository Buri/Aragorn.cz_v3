  ...
  if (isRozAdmin($uid) || isAdmin($uid) || isProgrammer($login)) {
    ...
    if ($xpcount!=-1 && $xpto!=-1 && $xpwhy!='') {
      ...
      if (!$result) {
        ...
        $result = mysql_query($query);
        if ($result){
			echo "OK - uživateli ".$_GET["xplogin"]." bylo přidáno ".$xpcount." XP";
			$fp = fopen('../xplog.txt', 'c+');
			$tries = 5;
			while(!flock($fp, LOCK_EX) && $tries){
				usleep(200);
				$tries--;
			}
			if($tries){
				fseek($fp, 0);
				$data = date('d.m.Y H:i - ') . '\n' . fread($fp, filesize('../xplog.txt'));
				ftruncate($fp);
				fseek($fp, 0);
				fwrite($fp, $data);
				fflush($fp);
				...
				echo "<br/> Nepodařilo se zapsat do logu.";
			}
          fclose($fp);
        }
