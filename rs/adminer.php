...
upload_error($j){$Zc=($j==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($j?'Nepodařilo se nahrát soubor.'.($Zc?" ".sprintf('Maximální povolená velikost souboru je %sB.',$Zc):""):'Soubor neexistuje.');}function
...
enum_input($V,$ua,$k,$Y,$Bb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$k["length"],$Uc);$J=($Bb!==null?"<label><input type='$V'$ua value='$Bb'".((is_array($Y)?in_array($Bb,$Y):$Y===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($Uc[1]as$r=>$X){$X=stripcslashes(str_replace("''","'",$X));$Ha=(is_int($Y)?$Y==$r+1:(is_array($Y)?in_array($r+1,$Y):$Y===$X));$J.=" <label><input type='$V'$ua value='".($r+1)."'".($Ha?' checked':'').'>'.h($b->editVal($X,$k)).'</label>';}return$J;}function
input($k,$Y,$o){global$Af,$b,$w;$D=h(bracket_escape($k["field"]));echo"<td class='function'>";$ve=($w=="mssql"&&$k["auto_increment"]);if($ve&&!$_POST["save"])$o=null;$ic=(isset($_GET["select"])||$ve?array("orig"=>'původní'):array())+$b->editFunctions($k);$ua=" name='fields[$D]'";if($k["type"]=="enum")echo
...
selectLinks($Ze,$P=""){echo'<p class="tabs">';$Rc=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($Ze))$Rc["view"]='Pozměnit pohled';else$Rc["create"]='Pozměnit tabulku';if($P!==null)$Rc["edit"]='Nová položka';foreach($Rc
...
selectColumnsPrint($N,$e){global$ic,$lc;print_fieldset("select",'Vypsat',$N);$r=0;$hc=array('Funkce'=>$ic,'Agregace'=>$lc);foreach($N
...
selectOrderPrint($Bd,$e,$u){print_fieldset("sort",'Seřadit',$Bd);$r=0;foreach((array)$_GET["order"]as$x=>$X){if(isset($e[$X])){echo"<div><select name='order[$r]' onchange='selectFieldChange(this.form);'><option>".optionlist($e,$X,true)."</select>",checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'sestupně')."</div>\n";$r++;}}echo"<div><select name='order[$r]' onchange='selectAddRow(this);'><option>".optionlist($e,null,true)."</select>","<label><input type='checkbox' name='desc[$r]' value='1'>".'sestupně'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($kf){if($kf!==null){echo"<fieldset><legend>".'Délka textů'."</legend><div>",'<input name="text_length" size="3" value="'.h($kf).'">',"</div></fieldset>\n";}}function
...
edit_type($x,$k,$d,$n=array()){global$Se,$Af,$Hf,$vd;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$k["type"]||isset($Af[$k["type"]])?array():array($k["type"]))+$Se+($n?array('Cizí klíče'=>$n):array()),$k["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($k["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$k["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($d,$k["collation"]).'</select>',($Hf?"<select name='$x"."[unsigned]'".(!$k["type"]||ereg('(int|float|double|decimal)$',$k["type"])?"":" class='hidden'").'><option>'.optionlist($Hf,$k["unsigned"]).'</select>':''),($n?"<select name='$x"."[on_delete]'".(ereg("`",$k["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist(explode("|",$vd),$k["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Sa?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($l))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.1' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($l),';</script>
</thead>
...
',(support("comment")?"<td".($Sa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($k["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.1' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $ra, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.5.1' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.5.1' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($Fd==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.5.1' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$x=>$X)echo"<tr valign='top'><td>$X[0]<td>$X[1]<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'Změnit'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Přidat trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databáze',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$bf=array();$cf=array();$D="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$D-".DB]?"$D-".DB:$D)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Uc,PREG_SET_ORDER);foreach($Uc
...
as$x=>$X){list($G,$lf)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.'Upravit'."</a> <span class='time'>".@date("H:i:s",$lf)."</span> <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$G)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistit'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upravit vše'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$D=>$k){if(!isset($k["privileges"][$If?"update":"insert"])||$b->fieldName($k)=="")unset($l[$D]);}if($_POST&&!$j&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($If?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$_))$_=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Položka byla smazána.');else{$P=array();foreach($l
as$D=>$k){$X=process_input($k);if($X!==false&&$X!==null)$P[idf_escape($D)]=($If?"\n".idf_escape($D)." = $X":$X);}if($If){if(!$P)redirect($_);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$P),"\nWHERE $Z"),$_,'Položka byla aktualizována.');}else{$I=insert_into($a,$P);$Jc=($I?last_id():0);queries_redirect($_,sprintf('Položka%s byla vložena.',($Jc?" $Jc":"")),$I);}}}$af=$b->tableName(table_status($a));page_header(($If?'Upravit':'Vložit'),$j,array("select"=>array($a,$af)),$af);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$N=array();foreach($l
...
';if($l){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($If?'Uložit a pokračovat v editaci':'Uložit a vložit další')."' title='Ctrl+Shift+Enter'>\n";}echo($If?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$l?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$Qd=ereg('RANGE|LIST',$K["partition_by"]);print_fieldset("partition",'Rozdělit podle',$K["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Pd,$K["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($K["partition"]),'">)
Oddíly: <input name="partitions" size="2" value="',h($K["partitions"]),'"',($Qd||!$K["partition_by"]?" class='hidden'":""),'>
...
as$i){if(count($h)==1||$i!=""){if(!create_database($i,$_POST["collation"]))$Te=false;$Ic=$i;}}queries_redirect(ME."db=".urlencode($Ic),'Databáze byla vytvořena.',$Te);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($D).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$j,array(),DB);$d=collations();$D=DB;$La=null;if($_POST){$D=$_POST["name"];$La=$_POST["collation"];}elseif(DB!="")$La=db_collation(DB,$d);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$A)&&$A[1]){$D=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<p>
Při smazání: ',html_select("on_delete",array(-1=>"")+explode("|",$vd),$K["on_delete"]),' Při změně: ',html_select("on_update",array(-1=>"")+explode("|",$vd),$K["on_update"]),'<p>
<input type="submit" value="Uložit">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_c,$K["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Qe,$K["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
as$X){if($X[1]!="USAGE")$jc["$A[2]$X[2]"][$X[1]]=true;if(ereg(' WITH GRANT OPTION',$K[0]))$jc["$A[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$A))$sd=$A[1];}}if($_POST&&!$j){$td=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$ld=q($_POST["user"])."@".q($_POST["host"]);$Td=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $td",ME."privileges=",'Uživatel byl odstraněn.');else{$cb=false;if($td!=$ld){$cb=queries(($f->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $ld IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Td");$j=!$cb;}elseif($_POST["pass"]!=$sd||!$_POST["hashed"])queries("SET PASSWORD FOR $ld = ".($_POST["hashed"]?$Td:"PASSWORD($Td)"));if(!$j){$we=array();foreach($kd
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($f->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$r=0;foreach($jc
as$od=>$p){echo'<th>'.($od!="*.*"?"<input name='objects[$r]' value='".h($od)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Xa=>$mb){foreach((array)$he[$Xa]as$ge=>$Ra){echo"<tr".odd()."><td".($mb?">$mb<td":" colspan='2'").' lang="en" title="'.h($Ra).'">'.h($ge);$r=0;foreach($jc
as$od=>$p){$D="'grants[$r][".h(strtoupper($ge))."]'";$Y=$p[strtoupper($ge)];if($Xa=="Server Admin"&&$od!=(isset($jc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$D><option><option value='1'".($Y?" selected":"").">".'Povolit'."<option value='0'".($Y=="0"?" selected":"").">".'Zakázat'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$j){$Fc=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Fc++;}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$Fc),$Fc||!$_POST["kill"]);}page_header('Seznam procesů',$j);echo'
<form action="" method="post">
...
as$D=>$X){$X=process_input($l[$D]);if($X!==null){if($_POST["clone"])$P[idf_escape($D)]=($X!==false?$X:idf_escape($D));elseif($X!==false)$P[]=idf_escape($D)." = $X";}}$H.=($_POST["clone"]?" (".implode(", ",array_keys($P)).")\nSELECT ".implode(", ",$P)."\nFROM ".table($a):" SET\n".implode(",\n",$P));}if($_POST["delete"]||$P){$Pa="UPDATE";if($_POST["delete"]){$Pa="DELETE";$H="FROM $H";}if($_POST["clone"]){$Pa="INSERT";$H="INTO $H";}if($_POST["all"]||($Gf===array()&&$_POST["check"])||count($q)<count($N)){$I=queries("$Pa $H".($_POST["all"]?($Z?"\nWHERE ".implode(" AND ",$Z):""):"\nWHERE $Uf"));$ma=$f->affected_rows;}else{foreach((array)$_POST["check"]as$X){$I=queries($Pa.limit1($H,"\nWHERE ".where_check($X)));if(!$I)break;$ma+=$f->affected_rows;}}}$B=lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$ma);if($_POST["clone"]&&$I&&$ma==1){$Jc=last_id();if($Jc)$B=sprintf('Položka%s byla vložena.'," $Jc");}queries_redirect(remove_from_uri("page"),$B,$I);}elseif(!$_POST["import"]){if(!$_POST["val"])$j='Dvojklikněte na políčko, které chcete změnit.';else{$I=true;$ma=0;foreach($_POST["val"]as$Ef=>$K){$P=array();foreach($K
as$x=>$X){$x=bracket_escape($x,1);$P[]=idf_escape($x)." = ".(ereg('char|text',$l[$x]["type"])||$X!=""?$b->processInput($l[$x],$X):"NULL");}$H=table($a)." SET ".implode(", ",$P);$Tf=" WHERE ".where_check($Ef).($Z?" AND ".implode(" AND ",$Z):"");$I=queries("UPDATE".(count($q)<count($N)?" $H$Tf":limit1($H,$Tf)));if(!$I)break;$ma+=$f->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$ma),$I);}}elseif(is_string($Vb=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($la["output"])."&format=".urlencode($_POST["separator"]));$I=true;$Na=array_keys($l);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Vb,$Uc);$ma=count($Uc[0]);begin();$He=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($Uc[0]as$x=>$X){preg_match_all("~((\"[^\"]*\")+|[^$He]*)$He~",$X.$He,$Vc);if(!$x&&!array_diff($Vc[1],$Na)){$Na=$Vc[1];$ma--;}else{$P=array();foreach($Vc[1]as$r=>$Ka)$P[idf_escape($Na[$r])]=($Ka==""&&$l[$Na[$r]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Ka))));$I=insert_update($a,$P,$de);if(!$I)break;}}if($I)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Byl importován %d záznam.','Byly importovány %d záznamy.','Bylo importováno %d záznamů.'),$ma),$I);queries("ROLLBACK");}else$j=upload_error($Vb);}}$af=$b->tableName($T);page_header('Vypsat'.": $af",$j);session_write_close();$P=null;if(isset($xe["insert"])){$P="";foreach((array)$_GET["where"]as$X){if(count($n[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!ereg('[_%]',$X["val"]))))$P.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($T,$P);if(!$e)echo"<p class='error'>".'Nepodařilo se vypsat tabulku'.($l?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($N,$e);$b->selectSearchPrint($Z,$e,$u);$b->selectOrderPrint($Bd,$e,$u);$b->selectLimitPrint($y);$b->selectLengthPrint($kf);$b->selectActionPrint($u);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$ec=$f->result("SELECT COUNT(*) FROM ".table($a).($Z?" WHERE ".implode(" AND ",$Z):""));$E=floor(max(0,$ec-1)/$y);}$H="SELECT".limit((+$y&&$q&&count($q)<count($N)&&$w=="sql"?"SQL_CALC_FOUND_ROWS ":"").$gc,($Z?"\nWHERE ".implode(" AND ",$Z):"").$kc,($y!=""?+$y:null),($E?$y*$E:0),"\n");echo$b->selectQuery($H);$I=$f->query($H);if(!$I)echo"<p class='error'>".error()."\n";else{if($w=="mssql")$I->seek($y*$E);$Ab=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$w=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last")$ec=(+$y&&$q&&count($q)<count($N)?($w=="sql"?$f->result(" SELECT FOUND_ROWS()"):$f->result("SELECT COUNT(*) FROM ($H) x")):count($L));if(!$L)echo"<p class='message'>".'Žádné řádky.'."\n";else{$za=$b->backwardKeys($a,$af);echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$q&&$N?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upravit'."</a>");$jd=array();$ic=array();reset($N);$me=1;foreach($L[0]as$x=>$X){if($x!=$qd){$X=$_GET["columns"][key($N)];$k=$l[$N?($X?$X["col"]:current($N)):$x];$D=($k?$b->fieldName($k,$me):"*");if($D!=""){$me++;$jd[$x]=$D;$Oa=idf_escape($x);$qc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($x);$mb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($qc.($Bd[0]==$Oa||$Bd[0]==$x||(!$Bd&&count($q)<count($N)&&$q[0]==$Oa)?$mb:'')).'">';echo(!$N||$X?apply_sql_function($X["fun"],$D):h(current($N)))."</a>";echo"<span class='column hidden'>","<a href='".h($qc.$mb)."' title='".'sestupně'."' class='text'> ↓</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($x)).'\'); return false;" title="'.'Vyhledat'.'" class="text jsonly"> =</a>';echo"</span>";}$ic[$x]=$X["fun"];next($N);}}$Pc=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$x=>$X)$Pc[$x]=max($Pc[$x],min(40,strlen(utf8_decode($X))));}}echo($za?"<th>".'Vztahy':"")."</thead>\n";foreach($b->rowDescriptions($L,$n)as$C=>$K){$Df=unique_array($L[$C],$u);$Ef="";foreach($Df
as$x=>$X)$Ef.="&".($X!==null?urlencode("where[".bracket_escape($x)."]")."=".urlencode($X):"null%5B%5D=".urlencode($x));echo"<tr".odd().">".(!$q&&$N?"":"<td>".checkbox("check[]",substr($Ef,1),in_array(substr($Ef,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($q)<count($N)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Ef)."'>".'upravit'."</a>"));foreach($K
