...
function terminateUser($uid, $rid, $ban = false){
	...
	if ($ban && isset($AragornCache)) {
		...
		mysql_query ("DELETE FROM 3_chat_users WHERE uid = $uid AND rid = $rid");
	}
					...
					if ($_POST['cmd'] == "ban") {
						...
						if ($error != "0") {
							...
							if ($s && mysql_num_rows($s)){
								...
								else echo "Nelze vyhodit admina";
							}
						...
						if (strlen($cmd)>5){
							ajaxChatInsertSystem($cmd, $_GET['id'], "Systém");
							echo "/sys OK";
						}
	...
	case "chat_sending":
		...
		if ($getColor === false) {
	 	...
	 	if (mb_strlen($mess)>0) {
			...
			if ($itIsApril && isset($_SESSION['chut']) && $_SESSION['chut'] == 1) {
				$mess = vypatlej($mess);
			}
